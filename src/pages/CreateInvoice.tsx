...
export default function CreateInvoice() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Format dates to ISO string, handling empty values
      const formattedInvoiceData = {
        ...invoiceData,
        user_id: "1", // TODO: Get actual user ID from auth context
        invoice_date: invoiceData.invoice_date ? new Date(invoiceData.invoice_date).toISOString() : new Date().toISOString(),
      ...
      // Remove invoice_number as it will be generated by the service
      delete formattedInvoiceData.invoice_number;
      ...
      const invoiceItems = selectedItems.map(item => {
        ...
        return {
          ...
          total_amount: lineTotal + cgstAmount + sgstAmount,
        };
      });

      // Calculate totals
      const subtotal = invoiceItems.reduce((sum, item) => sum + (item.rate * item.quantity), 0);
      const cgstTotal = invoiceItems.reduce((sum, item) => sum + item.cgst_amount, 0);
      const sgstTotal = invoiceItems.reduce((sum, item) => sum + item.sgst_amount, 0);
      ...
      await addInvoiceItems(Number(invoice.id), invoiceItems.map(item => ({
        ...item,
  ...
  const handleQuantityChange = (itemId: number, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const invoiceItems = selectedItems.map(item => {
      ...
      const quantity = itemId === itemIdNum ? value : itemQuantities[itemIdNum] || 0;
      const lineTotal = item.value * quantity;
  ...
  const renderItemRow = (item: InventoryItem) => {
    const itemIdNum = Number(item.id);
    const quantity = itemQuantities[itemIdNum] || 0;
      ...
      <TableRow key={item.id}>
        ...
        <TableCell className="text-right">
          <Input
            ...
            max={item.stock}
            value={quantity}
            onChange={(e) => handleQuantityChange(itemIdNum, e)}
